...
// Color Edge Marking (彩色边缘标记)
//
// Extends the base Sobel edge display with configurable color overlay:
//   - Non-edge pixels -> black background (0, 0, 0)
//   - Edge pixels     -> highlighted in the selected color
//
// COLOR_SEL parameter selects the highlight color:
//   0 = RED       (edge intensity modulates R, G=B=0)
//   1 = GREEN     (edge intensity modulates G, R=B=0)
//   2 = BLUE      (edge intensity modulates B, R=G=0)
//
// EDGE_THRESHOLD sets the minimum edge value to be shown as color.
...
// === Color edge marking parameters ===
parameter [1:0] COLOR_SEL     = 2'd0;   // 0=RED, 1=GREEN, 2=BLUE
parameter [7:0] EDGE_THRESHOLD = 8'd48;  // below this -> background (black)

// === Timing constants (1280x720p) ===
...
// === Color edge marking RGB output ===
// Non-edge pixels -> black background
// Edge pixels -> colored with intensity proportional to edge strength
wire edge_is_strong;
...
// === BRAM interface (read-only from PL side) ===
